...
void aStar::computePath()
{
    ...
    while (!this->openList.empty())
    {
        ...
        for (int i = 0; i < NUMOFDIRS + 1; i++)
        {
            ...
            if ((newx >= 1 && newx <= x_size && newy >= 1 && newy <= y_size && ((int)map[GETMAPINDEX(newx, newy, x_size, y_size)] < collision_thresh) && (curr_time + time_elapsed + newt) <= target_steps)) {
                ...
                if (newx== target_x && newy== target_y)/* || (newx==target_traj[target_steps-1] && newy==target_traj[2*target_steps-1])*/ //if new pose is the goal pose at that time
                {
...

                {
                    if (cellInfo[{newx, newy, newt}].g > cellInfo[{k, j, t}].g + this->map[xyToIndex(newx, newy)]) //If g of new > g of curr + cost of new
                    {
                        ...
                        int time_elapsed_loop = (int)((clock() - start) / CLOCKS_PER_SEC);
                        cellInfo[{newx, newy, newt}].h =  100000*((target_steps-time_elapsed_loop)+cellInfo2D[xyToIndex(newx, newy)].g);  //min(h_n,cellInfo2D[xyToIndex(newx,newy)].g);
